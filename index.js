...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur) {
  ...
  const anahtarlar = [1, 3, 5, 7, 9];
  const paragraf = cumleKur(
    ...anahtarlar.map((anahtar) => cumleDizisi[anahtar])
  );
  return paragraf;
}
...
var manav = meyveler.slice();
manav = manav.concat(sebzeler);
...
function emojileriDonustur(mesaj, emojiler) {
  for (let sembol in emojiler) {
    const sembolRegExp = new RegExp(
      sembol.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&"),
      "gi"
    );
    mesaj = mesaj.replace(sembolRegExp, emojiler[sembol]);
  }
